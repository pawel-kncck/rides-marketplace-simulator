...
class Matcher:
    ...
    def process_order(self, rider: RiderAgent, fare: float, order_id: str) -> Tuple[Optional[DriverAgent], str]:
        """
        ...
        """
        logging.info(f"Matcher received Order {order_id} from Rider {rider.agent_id}. Searching for drivers.")
        idle_drivers = self.find_nearest_idle_drivers(rider)

        if not idle_drivers:
            logging.warning(f"Order {order_id}: Failed to match. Reason: UNFULFILLED_NO_DRIVERS.")
            return None, "UNFULFILLED_NO_DRIVERS"

        for i, driver in enumerate(idle_drivers):
            if i >= self.max_order_tries:
                logging.warning(f"Order {order_id}: Failed to match. Reason: UNFULFILLED_MAX_TRIES.")
            ...
            logging.info(f"Order {order_id}: Attempting Driver {driver.agent_id} at {driver.location} for Rider {rider.agent_id} at {rider.location} (Profitability Score: {profitability_score:.2f}).")

            if profitability_score > 0:
                logging.info(f"Order {order_id}: Driver {driver.agent_id} ACCEPTED the offer.")
                return driver, "MATCH_SUCCESSFUL"
            else:
                logging.info(f"Order {order_id}: Driver {driver.agent_id} REJECTED the offer (score {profitability_score:.2f} <= 0).")

        logging.warning(f"Order {order_id}: Failed to match after trying all available drivers. Reason: UNFULFILLED_NO_DRIVERS.")
        return None, "UNFULFILLED_NO_DRIVERS"
