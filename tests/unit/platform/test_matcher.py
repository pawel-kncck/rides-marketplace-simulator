...
def test_matcher_stops_on_acceptance(grid, rider):
    """Tests that the matcher stops after the first acceptance."""
    # Ensure the first driver will accept
    driver1 = grid.get_agents_in_cell(grid.get_cell_id((5,5)))[0]
    driver1.price_sensitivity = 0.9 # Will accept a fare of 10

    # Add another idle driver who would also accept
    driver2 = DriverAgent(4, (7, 7), False, 0.9, 0.5, 0.5)
    driver2.current_state = DriverStateEnum.IDLE
    grid.add_agent(driver2)
    ...
    assert status == "MATCH_SUCCESSFUL"
    assert matched_driver.agent_id == 1
...
def test_matcher_unfulfilled_if_no_drivers(rider):
    ...
    matched_driver, status = matcher.process_order(rider, fare=20)
